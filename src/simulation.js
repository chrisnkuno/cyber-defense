...
export function listProfiles(dataset) {
  return dataset.profiles.map((profile) => ({
    id: profile.id,
    name: profile.name,
    role: profile.role,
    ticks: profile.timeline.length,
    latestRiskScore: profile.timeline.at(-1).riskScore,
  }));
}
...
export function buildDashboardState(dataset, profileId, tick) {
  ...
  const recommendations = buildRecommendations(current.state, current.scenarios);
