...
export function stateVector(state) {
  return FEATURE_KEYS.map((key) => {
    ...
    if (key === "mfaCoverage" || key === "backupReadiness") {
      return 1 - value;
    }
    return value;
  });
...
export function riskTrajectory(timeline, currentTick) {
  if (currentTick === 0) {
    return { direction: "stable", delta: 0 };
  }

  const previous = timeline[currentTick - 1].riskScore;
  const current = timeline[currentTick].riskScore;
